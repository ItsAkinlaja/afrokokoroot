...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-white/80 backdrop-blur-xl shadow-sm transition-all duration-300">
      ...
      <div 
        className={cn(
          "fixed inset-0 z-40 bg-white/95 backdrop-blur-xl lg:hidden transition-all duration-300 ease-in-out transform",
          mobileMenuOpen ? "translate-x-0 opacity-100" : "translate-x-full opacity-0 pointer-events-none"
        )}
      >
        <div className="container h-full flex flex-col justify-center items-center space-y-8 pt-20 pb-10">
          <div className="flex flex-col items-center space-y-6 w-full max-w-md">
            {navigation.map((item, i) => (
              <Link
                ...
                href={item.href}
                className={cn(
                  "text-2xl font-black transition-all hover:text-orange-600 hover:scale-105 py-2 px-4 rounded-xl",
                  pathname === item.href
                    ? "text-orange-600 bg-orange-50"
                    : "text-slate-800"
                )}
